...
class ChatOrchestrator:
    ...
    and memory from RedisMemoryService before invoking Groq.
    """
    ...
    async def _manage_token_window(self, raw_history: list) -> list:
        ...
        try:
            ...
            ack_node = {
                "role": "assistant",
                "content": "I have logged the summary of our previous conversation."
            }
    ...
    async def process_message(self, message: str, file_hash: str, session_id: str) -> str:
        ...
        maintain history, and return the AI response synchronously.
        """
        ...
        if not raw_history:
            ...
            if resume_context_string:
                ...
                raw_history.append({"role": "user", "content": context_message})
                raw_history.append({"role": "assistant", "content": "Understood. I will use this resume context to answer the user's career questions."})
        
        # 2.5 Manage token window
        ...
        raw_history.append({"role": "assistant", "content": response_text})
    ...
    async def stream_message(self, message: str, file_hash: str, session_id: str, is_interview: bool = False):
        """
        Process the user's message and yield a streaming response via Groq.
        """
        ...
        if not raw_history:
            ...
            if resume_context_string:
                ...
                raw_history.append({"role": "assistant", "content": initial_ack})
        ...
        try:
            ...
            logger.error(f"Groq API timeout during chat stream for session {session_id}.")
